...
Copyright (c) 2013 Michael Fenton, Jonathan Byrne,
Erik Hemberg, James McDermott
...
class StructuralFitness():
    ...
    def __call__(self, name, unique_id, program, genome, used_codons_a,
                 used_codons_b, ave, mats, generation, LOAD, MATERIALS_FILE,
                 FITNESSES, DEBUG = False, PRINT = False):
        import analyser as AZR
        analyser = AZR.Analyser(name, unique_id, program, genome,
                used_codons_a, used_codons_b, ave, mats, generation,
                LOAD, MATERIALS_FILE)
        total_fitness = analyser.test_mesh(FITNESSES, LOAD,
                                           DEBUG, False, PRINT)
        fitness_results = []
...
class Individual(object):
    ...
    def __init__(self, genome_a, genome_b, length=800):
        if genome_a == None and genome_b == None:
            self.genome_a = [randint(0, CODON_A_SIZE)
                           for _ in xrange(length)]
            genome_b = randint(0, CODON_B_SIZE)
            self.genome_b = [genome_b for _ in xrange(length)]
        else:
            self.genome_a = deepcopy(genome_a)
            self.genome_b = deepcopy(genome_b)
        self.name = None
    ...
    def __str__(self):
        return ("Individual: " + " uid: " + str(self.uid)
                + str(self.genome_a) + str(self.genome_b) + "; "
                + str(self.fitness))

    def evaluate(self, name, fitness, ave, mats, generation, LOAD,
    ...
    def save_result(self, result):
        ...
        self.name, self.fitness = result[0], result[1]
        self.phenotype, self.used_codons_a = result[2], result[3]
        self.used_codons_b, self.node_list = result[4], result[5]
        self.edge_list, self.good = result[6], result[7]

def initialise_population(size=10):
    """Create a popultaion of size and return"""
    return [Individual(None, None) for _ in xrange(size)]

def print_stats(generation, individuals, best_ever, mats, best_nos,
                MATERIALS_FILE, DEBUG = False):
    ...
    if use_prob:
        for i in xrange(len(genome)):
            if random() < MUTATION_PROBABILITY:
                genome[i] = randint(0, CODON_SIZE)
    else:
        idx = randint(0, codons - 1)
...
def parallelize_indivs(bind):
    ...
    name, fitness, ind = bind[0], bind[1], bind[2]
    grammar, ave, mats, generation = bind[3], bind[4], bind[5], bind[6]
    LOAD, MATERIALS_FILE, FITNESSES = bind[7], bind[8], bind[9]
    DEBUG, PRINT = bind[10], bind[11]
    values = grammar.generate(ind.genome_a, ind.genome_b)
    ind.set_values(values)
    if ind.phenotype != None:
        everything = ind.evaluate(name, fitness, ave, mats, generation, LOAD,
                                  MATERIALS_FILE, FITNESSES, DEBUG, PRINT)
    else:
...
def step(percent, counter, previous_best, max_gens, parent_pop, grammar,
         selection, best_ever, fitness, ave, mats, generation, MATERIALS_FILE,
         LOAD, FITNESSES, DEBUG = False, MULTI_CORE = True):
    ...
    while len(child_pop) < GENERATION_SIZE:
        child_pop.extend(onepoint_crossover(*sample(parents, 2)))
    ...
    for i in papal_top:
        i.ave = ave
    evaluate_fitness(papal_top, grammar, fitness, ave, mats, generation,
                     MATERIALS_FILE, LOAD, FITNESSES, DEBUG, MULTI_CORE)
    p = max(elites)
    for i, ind in enumerate(elites):
        ind.uid = i
    pre_elites = deepcopy(elites)
    for i, elite in enumerate(elites):
        evaluate_fitness([elites[i]], grammar, fitness, elite.ave, mats,
            generation, MATERIALS_FILE, LOAD, FITNESSES, DEBUG, MULTI_CORE)
    ...
    if MAKE_GIF:
        west_Mesh = AZR.Analyser(best_ever.name, "best",
                    str(best_ever.phenotype), best_ever.genome_b,
                    best_ever.used_codons_a, best_ever.used_codons_b, ave,
                    MATS, generation, LOAD, MATERIALS_FILE)
        west_Mesh.create_graph(str(TIME_STAMP), str(best_ever.phenotype),
                    best_ever.genome_b, LOAD, FITNESSES, DEBUG, FINAL=True)
...
def assign_size(MATERIALS_FILE):
    ...
    number = 0
    beams = []
    if MATERIALS_FILE == "CSSTables" or MATERIALS_FILE == "test1":
    # Units in lb (Imperial)
        for i, line in enumerate(tables):
            if line.startswith('#'):
                ...
                density = 2.76799 * 10 ** (-6)  #7.4182132 * 10 ** (-6)  
                #  in kg/millimeter cubed
                unitweight = area * density # in kg per millimeter
                one, one_point_five, two, two_point_five = 1, 1.5, 2, 2.5
                three, three_point_five, four, five, six = 3, 3.5, 4, 5, 6
                seven, eight, nine, ten = 7, 8, 9, 10
                material_properties = {'id':idx, '1':one, '1.5':one_point_five,
                                        '2':two, '2.5':two_point_five,
                                        '3':three, '3.5':three_point_five,
        ...
        for i, line in enumerate(tables):
            if line.startswith('#'):
                ...
                area = float(line[3])* 100 # in millimeters squared
                iy = float(line[5])*10000 # in millimeters to the four
                emod = 210000 # in Newtons/millimeters or MegaPascals
                density = 7.85 * 10 ** (-6)  # in kg/millimeter cubed
                if float(diameter) > 270:
                    two, three = float(line[11]), float(line[12])
                    four, five = float(line[13]), float(line[14])
                    six, seven = float(line[15]), float(line[16])
                    eight, nine = float(line[17]), float(line[18])
                    ten, eleven = float(line[19]), float(line[20])
                    twelve, thirteen = float(line[21]), float(line[22])
                    fourteen, one = float(line[23]), 0
                    one_point_five, two_point_five, three_point_five = 0, 0, 0
                else:
                    one, one_point_five =  float(line[11]), float(line[12])
                    two, two_point_five = float(line[13]), float(line[14])
                    three, three_point_five = float(line[15]), float(line[16])
                    four, five = float(line[17]), float(line[18])
                    six, seven = float(line[19]), float(line[20])
                    eight, nine = float(line[21]), float(line[22])
                    ten, eleven = float(line[23]), 0
                    twelve, thirteen, fourteen = 0, 0, 0
                material_properties = {'id':idx, '1':one, '1.5':one_point_five,
                                        '2':two, '2.5':two_point_five,
                                        '3':three, '3.5':three_point_five,
                                        '4':four, '5':five, '6':six, '7':seven,
                                        ...
                                        'diameter':diameter, 'I':iy,
                                        'thickness':thickness, 'emod':emod,
                                        'unitweight':unitweight,
...
def search_loop(fitness_function, max_gens, individuals, grammar, selection,
                FITNESSES, DEFAULT_FIT, MATERIALS_FILE, LOAD, MATS,
                DEBUG = False, MULTI_CORE = True):
    """Loop over max generations"""
    #Evaluate initial population
    ave = False
    evaluate_fitness(individuals, grammar, fitness_function, ave, MATS, 1,
                     MATERIALS_FILE, LOAD, FITNESSES, DEBUG, MULTI_CORE)
    ...
    if MAKE_GIF:
        west_Mesh = AZR.Analyser(best_ever.name, "best",
                    str(best_ever.phenotype), best_ever.genome_b,
                    best_ever.used_codons_a, best_ever.used_codons_b, ave,
                    MATS, 1, LOAD, MATERIALS_FILE)
        west_Mesh.create_graph(str(TIME_STAMP), str(best_ever.phenotype),
                    best_ever.genome_b, LOAD, FITNESSES, DEBUG, FINAL=True)
        west_Mesh.create_mesh(str(1))
    last_ten = [12344587, 9876984732865, 432534458709, 987987958755,
                3452089995, 1343125324, 13432424242, 9860980986,
                465436665 ,best_ever.fitness[0]]
    ...
    have = print_stats(1, individuals, best_ever, MATS, hound,
                       MATERIALS_FILE, DEBUG)
    SWITCH = False
    for generation in xrange(2, (max_gens + 1)):
        ...
        individuals, best_ever, best_nos, percent = step(
            percentage, counter, previous_best, max_gens, 
            individuals, grammar, selection, best_ever, fitness_function, 
            ave, MATS, generation, MATERIALS_FILE, LOAD, FITNESSES, 
            DEBUG, MULTI_CORE)
        last_ten.pop(0)
        ...
        if previous_best == 0:
            counter +=1
        have = print_stats(generation, individuals, best_ever, MATS, best_nos,
                           MATERIALS_FILE, DEBUG)
...
# GE Properties
GRAMMAR_FILE = "grammars/Delaunay_cantilever.bnf"
...
def run_all():
    ...
    if not path.isdir("/home/michael/Dropbox/Collij/Mike/truss/EliteResults"):
        mkdir("/home/michael/Dropbox/Collij/Mike/truss/EliteResults")
    if path.isdir("/home/michael/Dropbox/Collij/Mike/truss/population"):
        rmtree("/home/michael/Dropbox/Collij/Mike/truss/population")
    mkdir("/home/michael/Dropbox/Collij/Mike/truss/population")
    if path.isdir("/home/michael/Dropbox/Collij/Mike/truss/slf"):
        rmtree("/home/michael/Dropbox/Collij/Mike/truss/slf")
    mkdir("/home/michael/Dropbox/Collij/Mike/truss/slf")
    if path.isdir("/home/michael/Dropbox/Collij/Mike/truss/Pics"):
        rmtree("/home/michael/Dropbox/Collij/Mike/truss/Pics")
    mkdir("/home/michael/Dropbox/Collij/Mike/truss/Pics")
    global MATS, MULTI_CORE, DEBUG, POPULATION_SIZE, FITNESS_FUNCTION
    global GENERATIONS, ELITE_SIZE, FITNESSES, DEFAULT_FIT, GENERATION_SIZE
    global MUTATION_PROBABILITY, CROSSOVER_PROBABILITY, ACTUAL_GRAMMAR, now
    global hms, TIME_STAMP, SHOW_FINAL, SAVE_DXF, SAVE_BEST, LOAD
    global GRAMMAR_FILE, MATERIALS_FILE, THICKNESS_LIMITER, CODON_A_SIZE
    global CODON_B_SIZE
    now = datetime.now()
    hms = "%02d%02d%02d" % (now.hour, now.minute, now.second)
    TIME_STAMP = (str(now.day) + "_" + str(now.month) + "_" + hms)
    time1 = datetime.now()
    print "start:", time1
    parser = OptionParser(usage="if nothing is specified, it uses the default"\
                                "values specified in evolver class")
    ...
    # Loop
    LAST_POP, ave = search_loop(FITNESS_FUNCTION, GENERATIONS, INDIVIDUALS,
        BNF_GRAMMAR, tournament_selection, FITNESSES, DEFAULT_FIT,
        MATERIALS_FILE, LOAD, MATS, DEBUG, MULTI_CORE)
    time2 = datetime.now()
    print "end:", time2
    total_time = time2 - time1
    print "\ntime taken:", total_time
    print "\nBest fitness value:", LAST_POP.fitness,"\n"
  #  print LAST_POP.genome_a[:LAST_POP.used_codons_a]
    print LAST_POP.genome_b[:LAST_POP.used_codons_b]
    best_Mesh = AZR.Analyser(LAST_POP.name, "best", str(LAST_POP.phenotype),
        LAST_POP.genome_b, LAST_POP.used_codons_a, LAST_POP.used_codons_b,
        ave, MATS, GENERATION_SIZE, LOAD, MATERIALS_FILE)
    ...
    if not DEBUG:
        ...
        savefile.write("\nBest fitness values: " + str(LAST_POP.fitness
                       ...
                       "pulation Size = " + str(POPULATION_SIZE) + "\n# Gener"\
                       "ation Size = " + str(GENERATION_SIZE) + "\n# Generati"\
                       "ons = " + str(GENERATIONS) + "\n# Mutation = " + str(
        ...
        for i in range(len(FITNESSES)):
            savefile.write("\n# Fitness " + str(i+1) +": " + str(
                           best_Mesh.fitness_selections[FITNESSES[i]+ 1]))
        best_Mesh.create_graph(str(TIME_STAMP), str(LAST_POP.phenotype),
                        LAST_POP.genome_b, LOAD, FITNESSES, DEBUG, FINAL=True)
        ...
        if best_Mesh.UDL:
            ...
            savefile.write("\n# Load: " + str(best_Mesh.point_load) + " N Poi"\
                           "nt Load")
